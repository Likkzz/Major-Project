...
# Load saved calibration on startup (overrides hardcoded defaults if file exists)
...
def analyze_thread_signal(profile_x, profile_y):
    ...
    valleys_idx, _ = signal.find_peaks(-normalized_signal, height=1.0, distance=search_distance)
    peaks_idx, _ = signal.find_peaks(normalized_signal, height=1.0, distance=search_distance)
...
def process_auto_metrics(image_path):
    ...
    if thread_data:
        ...
        # --- Pitch Logic: Middle 5 Valleys ---
        valleys_for_pitch = valleys_idx
        count = len(valleys_idx)
        
        if count == 3:
            valleys_for_pitch = valleys_idx[1:]
        elif count == 4:
            valleys_for_pitch = valleys_idx[1:3]
        elif count == 5:
            valleys_for_pitch = valleys_idx[1:4]
        elif count > 5:
            mid_idx = count // 2
        ...
        # Calculate distance between lines for depth
        ...
        used_valley_indices = set(valleys_for_pitch)

        all_points = {
            "peaks": map_back_points(peaks_idx, smooth_x, region_y),
            "valleys": map_back_points(valleys_idx, smooth_x, region_y, used_valley_indices),
            "line_peaks": map_back_lines(vis_line_p),
            "line_valleys": map_back_lines(vis_line_v)
        }
    
    # Core Dia = Outer Dia - (2 * Depth)
...
@app.route('/get_spec/<name>', methods=['GET'])
def get_spec(name):
    db = load_db()
    if name in db:
...
def recalibrate():
    ...
    try:
        ...
        known_od     = float(request.form.get('known_od', 0))

        if not file or known_length <= 0 or known_od <= 0:
            return jsonify({'error': 'Missing file or known dimensions'}), 400

        filename = 'calibration_ref.' + file.filename.rsplit('.', 1)[-1]
        filepath = os.path.join(app.config['UPLOAD_FOLDER'], filename)
        file.save(filepath)
        ...
        try:
            ...
            print(f"BG removal failed during calibration: {e}")

        # Measure the reference workpiece with current (old) scales
        metrics = process_auto_metrics(filepath)
        if not metrics:
            return jsonify({'error': 'Could not process calibration image'}), 500

        measured_length_mm = metrics['length_mm']   # mm using OLD scale
        measured_od_mm     = metrics['od_mm']        # mm using OLD scale

        # Convert measured mm back to pixels using OLD scales
        length_px = measured_length_mm / SCALE_AXIS
        od_px     = measured_od_mm     / SCALE_PERP

        if length_px <= 0 or od_px <= 0:
            return jsonify({'error': 'Could not derive pixel measurements'}), 500

        # Derive new scales
        new_scale_axis = known_length / length_px
        new_scale_perp = known_od     / od_px

        # Persist and apply
        save_calibration(new_scale_axis, new_scale_perp)
        SCALE_AXIS = new_scale_axis
        SCALE_PERP = new_scale_perp

        return jsonify({
            'success': True,
            'new_scale_axis': round(new_scale_axis, 8),
            'new_scale_perp': round(new_scale_perp, 8),
            'derived_length_px': round(length_px, 2),
            'derived_od_px':     round(od_px, 2),
        })
